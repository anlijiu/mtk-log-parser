package io.github.treesitter.jtreesitter.androidlog;

import java.lang.foreign.*;

public final class TreeSitterAndroidlog {
    ...
    private static final TreeSitterAndroidlog INSTANCE = new TreeSitterAndroidlog();

    private final Arena arena = Arena.ofAuto();
    private volatile SymbolLookup lookup = null;

    private TreeSitterAndroidlog() {}
    ...
    public static MemorySegment language(SymbolLookup lookup) {
        return call(lookup, "tree_sitter_androidlog");
    }

    private SymbolLookup findLibrary() {
        try {
            var library = System.mapLibraryName("tree-sitter-androidlog");
            return SymbolLookup.libraryLookup(library, arena);
        } catch (IllegalArgumentException ex1) {
            try {
                System.loadLibrary("tree-sitter-androidlog");
